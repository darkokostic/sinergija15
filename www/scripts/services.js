...
.factory('MSSinergijaFactory', function($http, $localStorage) {
    //SETTINGS
    var url = 'https://www.mssinergija.net:5567/Agenda.ashx?op=Agenda';
    var SinergijaFactory = {};
    //END - SETTINGS

    SinergijaFactory.getObject = function() {
        return $http.get(url, {
           headers: {}
           })
    ...
    SinergijaFactory.getTweets = function() {
        /*********************************************************************
    ...
    SinergijaFactory.getSessionOfSpeaker = function(speakerId) {
        for(var i = 0; i < $localStorage.data.SessionSpeakerRelations.length; i++) {
            if($localStorage.data.SessionSpeakerRelations[i].SpeakerId == speakerId) {
                for(var k = 0; k < $localStorage.data.Sessions.length; k++) {
                    if($localStorage.data.Sessions[k].Id == $localStorage.data.SessionSpeakerRelations[i].SessionId) {
                        return $localStorage.data.Sessions[k];
                    }
                }
            }
        }
    ...
    SinergijaFactory.getSpeakerOfSession = function(sessionId) {
        for(var i = 0; i < $localStorage.data.SessionSpeakerRelations.length; i++) {
            if($localStorage.data.SessionSpeakerRelations[i].SessionId == sessionId) {
                for(var k = 0; k < $localStorage.data.Speakers.length; k++) {
                    if($localStorage.data.Speakers[k].Id == $localStorage.data.SessionSpeakerRelations[i].SpeakerId) {
                        return $localStorage.data.Speakers[k];
                    }
                }
            }
        }
    ...
    SinergijaFactory.getLecturer = function(speakerId) {
        for(var i = 0; i < $localStorage.data.Speakers.length; i++) {
            if($localStorage.data.Speakers[i].Id == speakerId) {
                return $localStorage.data.Speakers[i];
            }
        }
    };

    SinergijaFactory.getLectures = function() {
        var lectures = new Array();
        for(var i = 0; i < $localStorage.data.Sessions.length; i++) {
            lectures.push($localStorage.data.Sessions[i]);
        }
        return lectures;
    };

    SinergijaFactory.getLectureById = function(lectureId) {
        for(var i = 0; i < $localStorage.data.Sessions.length; i++) {
            if($localStorage.data.Sessions[i].Id == lectureId) {
                return $localStorage.data.Sessions[i];
            }
        }
    };

    SinergijaFactory.getLectureByDay = function(day) {
        var lectures = SinergijaFactory.getLectures();
        var lecture = new Array();
        for(var i = 0; i < lectures.length; i++) {
            if(lectures[i].DayId == day) {
                lecture.push(lectures[i]);
            }
        }
        return lecture;
    };

    SinergijaFactory.getLecturesByTrack = function(track) {
        var lectures = SinergijaFactory.getLectures();
        var lecturesTrack = new Array();
        for(var i = 0; i < lectures.length; i++) {
            if(lectures[i].Track == track) {
                lecturesTrack.push(lectures[i]);
            }
        }
        return lecturesTrack;
    };

    SinergijaFactory.getLectureTime = function(lecture) {
        for(var i = 0; i < $localStorage.data.Slots.length; i++) {
            if($localStorage.data.Slots[i].TimeLine == lecture.TimeLine && $localStorage.data.Slots[i].Type == "Session" && lecture.DayId == $localStorage.data.Slots[i].DayId) {
                var time = "";
                if($localStorage.data.Slots[i].StartMinute == "0") {
                    time = $localStorage.data.Slots[i].StartHour + ":" + "00";
                } else {
                    time = $localStorage.data.Slots[i].StartHour + ":" + $localStorage.data.Slots[i].StartMinute;
                }
                return time;
            }
        }
    ...
    SinergijaFactory.getDateOfSession = function(session) {
        for(var i = 0; i < $localStorage.data.Days.length; i++) {
            if($localStorage.data.Days[i].Id == session.DayId) {
                return $localStorage.data.Days[i].Date;
            }
        }
    };

    SinergijaFactory.getRoomBySession = function(session) {
        for(var i = 0; i < $localStorage.data.Rooms.length; i++) {
            if($localStorage.data.Rooms[i].Id == session.RoomId) {
                return $localStorage.data.Rooms[i].Code;
            }
        }
    };

    return SinergijaFactory;
})
